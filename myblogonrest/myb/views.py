...
class FeedBackView(View):
    def get(self, request, *args, **kwargs):
        form = FeedBackForm()
        return render(request, 'myb/contact.html', context={
            'form': form,
            'title': 'Написать мне'
        })
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                send_mail(f'От {name} | {subject}', message, from_email, [str(os.getenv('MAIL'))])
...
class SuccessView(View):
    def get(self, request, *args, **kwargs):
        return render(request, 'myb/success.html', context={
            'title': 'Спасибо'
        })
...
class TagView(View):
    def get(self, request, slug, *args, **kwargs):
        ...
        common_tags = Post.tag.most_common()
        return render(request, 'myb/tag.html', context={
            'title': f'#ТЕГ {tag}',
            'posts': posts,
            'common_tags': common_tags
        })
...
class SearchResultsView(View):
    def get(self, request, *args, **kwargs):
        ...
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
        return render(request, 'myb/search.html', context={
            'title': 'Поиск',
            'results': page_obj,
